...
$url = "https://shootnn.yalla-shoot-tv.live/matches/$name/"; // الرابط الذي تريد جلب المحتوى منه
$newDomain = "matches.php?name="; // النطاق الجديد الذي تريد استبداله

// جلب المحتوى من الموقع
$content1 = file_get_contents($url);
$content = str_replace("Yalla Shoot", "Bashar", $content1);
if ($content === FALSE) {
...
foreach ($tags as $tag) {
    foreach ($dom->getElementsByTagName($tag) as $element) {
        if ($element->hasAttribute('href')) {
            ...
            $element->setAttribute('href', str_replace("https://yallateri.com/matches/", $newDomain, $urlAttr));
        }
...
$className = "coMatch"; // اسم الفئة الذي تريد جلب العناصر بناءً عليه
...
$url = "https://shootnn.yalla-shoot-tv.live/matches/$name/"; // الرابط الذي تريد جلب الصور منه
$newDomain = "matches.php?name="; // النطاق الجديد الذي تريد استبداله
...
$search = ["Yalla Shoot", "yalla shoot", "يلا شوت"]; // الكلمات التي تريد استبدالها
$replace = ["Bashar", "Bashar", "Bashar"]; // البدائل
...
foreach ($tags as $tag) {
    foreach ($dom->getElementsByTagName($tag) as $element) {
        ...
        if ($element->hasAttribute('src')) {
            ...
            $element->setAttribute('src', str_replace("https://yallateri.com", "kora1", $urlAttr));
        }
    }
}
